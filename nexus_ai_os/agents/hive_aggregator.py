"""
HiveAggregator — Synthesizes multiple model responses into consensus.
"""
import json
...
class HiveAggregator:
    """Agent that aggregates responses from multiple AI models."""
    ...
    async def aggregate(
        ...
        """Synthesize multiple model responses into a single consensus output."""
        logger.info(f"Hive Aggregator: Processing {len(hive_outputs)} responses...")
...
"""
        for i, res in enumerate(hive_outputs):
            if res.get('status') == 'success':
                synthesis_prompt += f"Model {i+1} ({res.get('provider', 'unknown')}):\n{res.get('content', '')}\n\n"
...
4. Output the final synthesized result only."""
        ...
        try:
            ...
            logger.info("Hive Consensus Synthesized.")
            return consensus_output
        except Exception as e:
            logger.error(f"Hive aggregation error: {e}")
            return "Error synthesizing consensus"
...
Output your verdict as a JSON with keys: 'quality_score' (0-10), 'consensus_strength' (LOW/MED/HIGH), and 'feedback' (1 sentence)."""
        ...
        try:
            ...
            logger.info(f"Hive Verdict: Score {review_data.get('quality_score', 'N/A')}/10 - Consensus: {review_data.get('consensus_strength', 'N/A')}")
            return review_data
        except Exception as e:
            logger.error(f"Hive Judge Error: {e}")
            return {"quality_score": 0, "consensus_strength": "LOW", "feedback": f"Judge failed: {e}"}
